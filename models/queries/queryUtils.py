...
def getTotalAmountAndWeeks(company_id, year, periodo):
    ...
    result = session.query(
      func.sum(Time.regular_pay + Time.over_pay + Time.vacation_pay + Time.meal_pay + Time.sick_pay + Time.holyday_pay)
        ...
        Employers.company_id == company_id,
        Time.pay_date >= period['start'],
        Time.pay_date <= period['end']
      ).scalar()
...
def getAmountVariosByCompany(company_id, year, period = None):
    ...
    result = session.query(
      ...
      func.sum(Time.tax_pr).label('taxes_pr')
          ...
          Employers.company_id == company_id,
          Time.pay_date >= date_start,
          Time.pay_date <= date_end
      ).group_by(Employers.id).all()
...
def getAmountVariosCompany(company_id, year, period = None):
    ...
    result = session.query(
      ...
      func.sum(Time.secure_social).label('secure_social'),
      func.sum(Time.tax_pr).label('taxes_pr')
      ).select_from(Period).join(Time, Period.id == Time.period_id ).join(Employers, Time.employer_id == Employers.id).filter( Employers.company_id == company_id,  Time.pay_date >= date_start ,Time.pay_date <= date_end ).all()
...
def getAmountVariosCompanyByMouth(company_id, year):
    ...
    result = session.query(
        ...
        extract('year', Time.pay_date).label('year'),
    extract('month', Time.pay_date).label('month'),
    ...
    .filter(Employers.company_id == company_id, Time.pay_date >= date_start, Time.pay_date <= date_end) \
    .group_by(extract('year', Time.pay_date), extract('month', Time.pay_date)) \
.order_by(extract('year', Time.pay_date), extract('month', Time.pay_date)) \
    .all()
    return result
...
def getBonusCompany(company_id, date_start, date_end,bonus):
    ...
    all_times_query = session.query(Time,Employers).select_from(Period).join(Time, Period.id == Time.period_id).join(Employers, Employers.id == Time.employer_id).filter(Time.pay_date >= date_start, Time.pay_date <= date_end, Employers.company_id == company_id).all()
...
def getAmountCSFECompany(company_id, date_start, date_end ):
    result = session.query(
     ...
     Time.employer_id.label('employer_id'),
      ...
      func.sum(Time.secure_social).label('secure_social'),
      func.sum(Time.tax_pr).label('taxes_pr')
      ).select_from(Period).join(Time, Period.id == Time.period_id ).join(Employers, Time.employer_id == Employers.id).filter( Employers.company_id == company_id,  Time.pay_date >= date_start , Time.pay_date <= date_end ).group_by(Time.employer_id).all()
...
def getAmountByCompany(company_id, date_start, date_end ):
    result = session.query(
      ...
      func.sum(Time.secure_social).label('secure_social'),
      func.sum(Time.tax_pr).label('taxes_pr')
      ).select_from(Period).join(Time, Period.id == Time.period_id ).join(Employers, Time.employer_id == Employers.id).filter( Employers.company_id == company_id,  Time.pay_date >= date_start , Time.pay_date <= date_end ).group_by(Employers.company_id).all()
...
def getAmountGroupEmployerWages(company_id, start , end ):
    ...
    result = session.query(
     ...
     Employers,
      ...
      func.sum(Time.secure_social).label('secure_social'),
      func.sum(Time.tax_pr).label('taxes_pr')
      ).select_from(Period).join(Time, Period.id == Time.period_id ).join(Employers, Time.employer_id == Employers.id).filter( Employers.company_id == company_id,  Time.pay_date >= start , Time.pay_date <= end ).group_by(Employers.id).all()
...
def getAmountGroupEmployer(company_id, year , month ):
    ...
    result = session.query(
      ...
      func.sum(Time.secure_social).label('secure_social'),
      func.sum(Time.tax_pr).label('taxes_pr')
      ).select_from(Period).join(Time, Period.id == Time.period_id ).join(Employers, Time.employer_id == Employers.id).filter( Employers.company_id == company_id,  Time.pay_date >= date_start , Time.pay_date <= date_end ).group_by(Employers.id).all()
...
def getAmountVariosCompanyGroupByMonth(company_id, year, period = None):
    ...
    result = session.query(
      ...
      func.sum(Time.secure_social).label('secure_social'),
      func.sum(Time.tax_pr).label('taxes_pr'),
      func.date_trunc('month', Time.pay_date).label('month'),
      ).join(Employers, Time.employer_id == Employers.id
      ).join(Period, Period.id == Time.period_id 
      
      ).filter( Employers.company_id == company_id, Time.pay_date >= date_start, Time.pay_date <= date_end 
      ).group_by(func.date_trunc('month', Time.pay_date)) \
  .having(func.date_trunc('month', Time.pay_date) <= date_end) \
  .order_by(func.date_trunc('month', Time.pay_date)).all()
